...
<p>
  This installation file directory contains the offline version of all the dependencies to ensure consistent versions are being used for all the files every time this lab is done.

  NOTE: Microsoft Edge (which is natively installed as part of the VM's operating system image) should be able to facilitate the downloading process of all the files but if any problems are experienced, just download and use the Google Chrome browser instead).
...
![image](https://github.com/user-attachments/assets/4a0ab37d-9fc0-4926-8136-94a140fc273c)
...
- From the [Installation Files](https://drive.google.com/drive/u/2/folders/1APMfNyfNzcxZC6EzdaNfdZsUwxWYChf6) download and install HeidiSQL
  ...
  - Launch HeidiSQL
  - Create a new session, use the username (root) and password that was assigned during the initial configuration of the MySQL server 
    instance
    - Notice that TCP port 3306 is being used by the HeidiSQL client to establish the connection to the MySQL server instance (as this is 
      the TCP/IP port that is used by default to connect to MySQL servers).
...
<p>
  ...
  - Set permissions to "Read" only for the "C:\inetpub\wwwroot\osTicket\include\ost-config.php" file
...
- Notes:
  ...
  - To browse to the osTicket End Users' ticket creation portal (within the VM): http://localhost/osTicket/
...
<h2>
  ...
  - PS: If you ever wanted to take a temporary break and reduce the costs incurred to have your computing resources hosted in Azure you can 
